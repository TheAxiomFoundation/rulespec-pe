...
ALLOWED_ROOT_DIRS = {".axiom", ".github", "bulk", "data", "programs", "tests", "pe"}
ALLOWED_ROOT_FILES = {
    ".gitignore",
    "CLAUDE.md",
    "README.md",
...
def test_only_peru_namespace_present() -> None:
    """Tanzania mainland scope: the only jurisdiction directory is pe/."""
    names = {d.name for d in jurisdiction_dirs()}
    assert names <= {"pe"}, f"unexpected jurisdiction dirs: {names - {'tz'}}"


def test_tz_content_buckets_exist() -> None:
    for marker in ("statutes", "regulations", "policies"):
        assert (ROOT / "pe" / marker).is_dir(), f"missing pe/{marker}"
...
def test_oracle_index_is_tz_scoped() -> None:
    ...
    assert payload["jurisdiction"] == "pe"
